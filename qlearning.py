...
class Qlearning:
    def __init__(self, problema, desconto=0.90, tetha=1e-6, alpha=0.1):
    ...
    def calcular_tabela_q(self, estado_inicial=0, n_passos=10000, limite_max=10):
        ...
        while passo < n_passos:
            if passo % 10000 == 0:
                print("%s passos de %s" % (passo, n_passos))
            passo += 1
            ...
            while limite < limite_max and ( self.problema.partida.venceu() == False or self.problema.partida.empate() == False) if len(self.problema.partida.historico) > 0 else None:
                ...
                for proximo_estado, probabilidade in self.problema.T(estado, acao):
                    q_seguinte += self.novo_q(
                        estado, acao, proximo_estado, probabilidade
                    )
                # aqui acontece a atualização do Q(s,a)
                self.Q[estado][acao] = (
                    self.alpha * q_antigo + (1 - self.alpha) * q_seguinte
                )
    ...
    def novo_q(self, estado, acao, proximo_estado, probabilidade):
        ...
        return probabilidade * (
            self.problema.R(estado, acao, proximo_estado) + self.desconto * max_a
        )
    ...
    def sorteia_proxima_acao(self, estado):
        ...
        return random.choices(
            [acao_random, acao_politica], weights=[self.e, (1 - self.e)]
        )[0]
